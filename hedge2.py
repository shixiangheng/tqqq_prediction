...
def execute_hedged_trades(data_tqqq, data_gld, initial_capital=10000.0):
    ...
    for date, row in data_tqqq.iterrows():
        ...
        if row['positions'] == 1.0 and cash > 0:
            ...
            cash = 0.0
            trades_info.append([date, 'BUY', 'TQQQ', position_tqqq, 0, cash])
        # Sell TQQQ and buy GLD
        elif row['positions'] == -1.0 and position_tqqq > 0:
            cash = position_tqqq * row['data']
            position_gld = cash / gld_row['data']
            trades_info.append([date, 'SELL', 'TQQQ', position_tqqq, cash, 0])
            cash = 0.0
            position_tqqq = 0.0
            trades_info.append([date, 'BUY', 'GLD', position_gld, 0, cash])
        
        # Update equity curve for TQQQ
        total_value = cash + position_tqqq * row['data'] + position_gld * gld_row['data']
        equity_curve.append(total_value)

    # Final update to sell GLD if still holding
    if position_gld > 0:
        final_gld_price = data_gld.iloc[-1]['data']
        cash = position_gld * final_gld_price
        trades_info.append([data_gld.index[-1], 'SELL', 'GLD', position_gld, cash, 0])
    ...
    # Convert trades_info to a DataFrame
    trades_df = pd.DataFrame(trades_info, columns=['Date', 'Action', 'Stock', 'Shares', 'Cash After Trade', 'Position After Trade'])
